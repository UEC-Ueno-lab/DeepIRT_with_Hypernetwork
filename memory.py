...
class MemoryHeadGroup():
    ...
    def write(self, value_memory_matrix, correlation_weight, embedded_content_vector, memory_matrix_pre_list,
        ...
        # updating process
        erase_signal = tf.sigmoid(
            self.linear_lstm(embedded_content_vector, value_memory_matrix_reshaped_hpre, self.memory_state_dim,
                             bias=True, scope=self.name + '/EraseOperation', ))

        zt_signal = tf.sigmoid(
            self.linear_lstm(embedded_content_vector, value_memory_matrix_reshaped_hpre, self.memory_state_dim,
                             bias=True, scope=self.name + '/ZtOperation', ))

        add_signal = tf.tanh(
            self.linear_lstm(value_memory_matrix_reshaped_hpre, zt_signal, self.memory_state_dim, bias=True,
